 ...
 * config('requestlogger.fields.url') === 'full'
 */
class MiddlewareFullUrlTest extends TestCase
{
    ...
    protected function getEnvironmentSetUp($app)
    {
        ...
        $app['config']->set('requestlogger.fields.url', 'full');
    ...
    public static function getScenarios(): array
    {
        return [
            ...
            'get-200-user_id'   => ['GET', '/check-request-logger?my-query=1', 'testUser', 200, 'route-without-params'],
            'get-200-user_none' => ['GET', '/hello-world?my-query=1' . rand(0, 1024), null, 200, null],
            'post-200-user_none'=> ['POST', '/hello-world?my-query=1' . rand(0, 1024), null, 200, null],
            'get-404-user_none' => ['GET', '/hello-world?my-query=1' . rand(0, 1024), null, 404, null],
            'get-500-user_none' => ['GET', '/hello-world?my-query=1' . rand(0, 1024), null, 500, null],

            // method, path, user, status_code, route
            // Params won't be logged by default
            'get-200-route-with-params'   => ['GET', '/check-request-logger/123456', null, 200, 'route-with-params', null],
        ];
    }
    ...
    public function test_middleware($method, $path, $user = null, $status_code = null, $route = null, $route_params = null): void
    {
        ...
        $this->assertEquals($log->route_params, $route_params);
